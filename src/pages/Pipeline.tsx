...
export const Pipeline = () => {
  ...
  const handleStatusChange = (id: string, nextStatus: LoanStatus) => {
    dispatch(updateLoanStatusAsync({ id, status: nextStatus }));
  };

  // Group loans by status into columns
  const columnsData = useMemo(() => {
    const groups: Record<LoanStatus, Loan[]> = {
      Pending: [],
      Active: [],
      Closed: [],
      Defaulted: []
  ...
  if (isLoading) {
    return (
      <div className="flex h-[60vh] w-full flex-col items-center justify-center gap-4">
        ...
        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest animate-pulse">Loading Pipeline Systems...</p>
      </div>
    );
  }
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1.5 leading-normal">
              Manage borrower lifecycle phases, authorize disbursements, and record repayments through visual workflows.
            </p>
          </div>
          <div className="flex items-center gap-2 rounded-xl bg-slate-800/50 border border-slate-700/30 px-4 py-2.5">
            ...
            <span className="text-xs font-bold text-slate-200">State Engine Connected</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-premium flex items-center gap-4">
          ...
          <div>
            ...
            <h3 className="text-lg font-black text-slate-800 mt-0.5">{formatCurrency(stats.totalPendingValue)}</h3>
          </div>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-premium flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Active Book Capital</p>
            <h3 className="text-lg font-black text-slate-800 mt-0.5">{formatCurrency(stats.totalActiveValue)}</h3>
          </div>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-premium flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">NPA At Risk</p>
            <h3 className="text-lg font-black text-slate-800 mt-0.5">{formatCurrency(stats.totalDefaultedValue)}</h3>
          </div>
        </div>
      </div>
            ...
            >
              ...
              <div className="p-4 border-b border-slate-150 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 font-semibold mt-0.5">{column.subtitle}</p>
                </div>
                <span className={`text-[10px] font-extrabold px-2.5 py-1 rounded-full border ${column.badgeColor}`}>
                  {column.loans.length}
                </span>
              </div>
              ...
              <div className="flex-1 p-3 overflow-y-auto space-y-3">
                {column.loans.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-48 border-2 border-dashed border-slate-200 rounded-xl p-4 text-center">
                    ...
                    <p className="text-[10px] font-bold text-slate-400 uppercase mt-2">No loans</p>
                    <p className="text-[9px] text-slate-400 mt-0.5">Empty pipeline ledger stage</p>
                  </div>
                ) : (
                      ...
                      >
                        ...
                        <div>
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] font-extrabold text-slate-400">{loan.id}</span>
                            <StatusBadge status={loan.status} />
                          </div>
                          <h4 className="text-xs font-black text-slate-800 mt-1">{loan.borrowerName}</h4>
                          <p className="text-[10px] font-semibold text-slate-400 mt-0.5">
                            {loan.loanType} • {loan.tenure} Mos @ {loan.interestRate}%
                          </p>
                        </div>
                        ...
                        <div className="flex items-center justify-between border-t border-slate-100 pt-3">
                          <div>
                            ...
                            <p className="text-sm font-extrabold text-slate-800">{formatCurrency(loan.amount)}</p>
                          </div>
                          
                          <div className="text-right">
                            <p className="text-[9px] font-bold text-slate-400 uppercase">Risk Index</p>
                        ...
                        {loan.status === 'Pending' && (
                          <div className="flex gap-2">
                            ...
                            <button
                              onClick={() => handleStatusChange(loan.id, 'Closed')}
                        ...
                        {loan.status === 'Active' && (
                          <div className="flex gap-2">
                            ...
                            <button
                              onClick={() => handleStatusChange(loan.id, 'Defaulted')}
