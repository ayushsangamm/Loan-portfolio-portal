...
export const BorrowerProfile = () => {
  ...
  const { selectedBorrower, status: borrowerStatus } = useAppSelector((state) => state.borrowers);
  ...
  const columns: Column<Loan>[] = [
    ...
    { 
      header: 'Monthly EMI', 
      accessor: (loan) => (
        <span className="font-semibold text-slate-650">{formatCurrency(loan.emiAmount)}</span>
      )
    },
    { 
      header: 'Status', 
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3.5">
        ...
        <div>
          <div className="flex items-center gap-2.5">
            <span className="text-xs font-bold text-slate-400 tracking-wider uppercase">Borrower profile</span>
            <span className="h-3 w-[1px] bg-slate-250" />
            <span className="text-xs font-bold text-brand-600">{selectedBorrower.id}</span>
          </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-premium space-y-6">
          <div className="flex items-center gap-3 pb-4 border-b border-slate-150">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-800">{selectedBorrower.name}</h3>
          ...
          <div className="space-y-4 text-xs font-medium text-slate-600">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Verified Salary</span>
                <span className="font-bold text-slate-800">{formatCurrency(selectedBorrower.annualIncome)} <span className="text-[10px] text-slate-400 font-semibold">/ year</span></span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-premium flex flex-col justify-between">
          <div className="flex items-center gap-3 pb-4 border-b border-slate-150">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-800">Bureau Credit Quality</h3>
              <p className="text-[10px] font-semibold text-slate-400 tracking-wide uppercase">CIBIL equivalent rating</p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center py-4">
            ...
            <div className="relative flex h-36 w-36 items-center justify-center rounded-full border-12 border-slate-100 shadow-inner">
              ...
              <div 
                ...
                style={{ 
                  clipPath: `polygon(50% 50%, 50% 0%, ${
                    selectedBorrower.creditScore >= 750 ? '100% 0%, 100% 100%, 0% 100%, 0% 0%' :
                    selectedBorrower.creditScore >= 680 ? '100% 0%, 100% 100%, 50% 100%' :
                    selectedBorrower.creditScore >= 550 ? '100% 0%, 50% 50%' : '50% 0%, 50% 50%'
                  })` 
                }} 
              />
              <div className="z-10 flex flex-col items-center justify-center">
                ...
                <span className={`mt-2 rounded-md border px-2 py-0.5 text-[9px] font-black uppercase tracking-wider ${rating.color}`}>
                  {rating.label}
                </span>
              </div>
            </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-900 text-white p-6 shadow-premium flex flex-col justify-between">
          <div className="flex items-center gap-3 pb-4 border-b border-slate-850">
            ...
            <div>
              <h3 className="text-sm font-bold text-white">Aggregated Balances</h3>
              <p className="text-[10px] font-semibold text-slate-400 tracking-wide uppercase">Portfolio volume stats</p>
            </div>
          </div>
          ...
          <div className="space-y-4 py-4 text-xs font-semibold text-slate-400">
            <div className="flex items-center justify-between border-b border-slate-850 pb-2">
              ...
              <span className="font-bold text-slate-100">{formatCurrency(totalDisbursed)}</span>
            </div>

            <div className="flex items-center justify-between border-b border-slate-850 pb-2">
              <span>Outstanding Installment EMI</span>
              <span className="font-bold text-brand-400">{formatCurrency(activeEmiMonthly)} / mo</span>
            </div>

            <div className="flex items-center justify-between">
              <span>Linked Contracts</span>
              <span className="font-bold text-slate-100">{borrowerLoans.length} Loans</span>
            </div>
          </div>
          ...
          <div className="rounded-xl bg-slate-850/60 p-4 border border-slate-850 text-[10px] text-slate-400 leading-normal flex items-start gap-2.5">
            ...
            <span>
              Subject borrower profile matches underwriting policy regulations. Capital disbersals remain validated for further credit line expansion.
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-base font-bold text-slate-800 tracking-tight leading-none md:text-lg">Issued Loan Contracts</h3>
        </div>
