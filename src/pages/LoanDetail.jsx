...
export const LoanDetail = () => {
  ...
  const { selectedLoan, status: loanStatus } = useAppSelector(
    (state) => state.loans,
  );
  ...
  const getCreditScoreDetails = (score) => {
    if (score >= 750)
      return {
        label: "Excellent",
  ...
  const isLoading =
    loanStatus === "loading" || !selectedLoan || !selectedBorrower;
    ...
    <div className="space-y-6">
          ...
          <div className="space-y-4 text-xs font-medium text-slate-600">
            ...
            <div className="rounded-xl border border-slate-150 bg-slate-50/50 p-4">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`rounded-md border px-2 py-0.5 text-[10px] font-bold tracking-wide ${creditDetails.color}`}
                >
                  {creditDetails.label}
                </span>
              </div>
              <div className="mt-3 flex items-end gap-3">
                ...
                <div className="h-2 flex-1 rounded-full bg-slate-200 overflow-hidden mb-1">
                  <div
                    ...
                    style={{
                      width: `${((selectedBorrower.creditScore - 300) / 550) * 100}%`,
                    }}
                  />
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 rounded-2xl border border-slate-200 bg-white p-6 shadow-premium space-y-4">
          <div>
            <h3 className="text-sm font-bold text-slate-800 tracking-tight md:text-base">
              Amortization & Repayment Breakdown
            </h3>
            <p className="text-xs text-slate-400 leading-normal">
              Amortized month-wise schedule of principal, interest, and monthly
              outstanding balance
            </p>
          </div>
              ...
              <tbody className="divide-y divide-slate-150 font-medium">
                {selectedLoan.repaymentSchedule.map((p) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-emerald-650">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-premium flex flex-col">
          <div className="pb-4 border-b border-slate-150 mb-5">
            ...
            <p className="text-xs text-slate-400 leading-normal">
              Chronological timeline of receipts
            </p>
          </div>
            ...
            {selectedLoan.paymentHistory.length === 0 ? (
              <div className="flex h-40 w-full flex-col items-center justify-center text-center">
                ...
                <p className="text-[10px] text-slate-350 leading-normal">
                  No scheduled repayments have fallen due.
                </p>
              </div>
            ) : (
              // Map timeline
                  ...
                  <div className="text-xs">
                    <div className="flex items-center gap-1.5">
                      <span className="font-bold text-slate-800">
                        {formatCurrency(
                          item.amountPaid || selectedLoan.emiAmount,
                        )}
                      </span>
                      <span
                    ...
                    {item.notes && (
                      <p className="mt-1 text-[10px] text-slate-500 leading-normal bg-slate-50/60 rounded p-1.5 border border-slate-100">
                        {item.notes}
                      </p>
                    )}
                  </div>
