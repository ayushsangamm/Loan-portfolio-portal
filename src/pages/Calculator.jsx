...
export const Calculator = () => {
  // Input states
  const [principal, setPrincipal] = useState(500000); // 5 Lakhs default
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1.5 leading-normal">
              Formulate credit structures, compute EMIs, and simulate dynamic
              amortization ledgers in real-time.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-premium">
            <h3 className="text-base font-bold text-slate-800 tracking-tight leading-none mb-6">
              Simulation Inputs
            </h3>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div className="rounded-2xl border border-brand-100 bg-brand-50/40 p-5 shadow-sm text-center">
              <p className="text-[10px] font-bold text-brand-600/80 uppercase tracking-wider">
                Monthly installment (EMI)
              </p>
              <h2 className="text-2xl font-black text-brand-800 mt-2">
                {formatCurrency(calculations.emi)}
              </h2>
              <p className="text-[10px] text-slate-400 mt-1">
                Due every cycle month
              </p>
            </div>
            <div className="rounded-2xl border border-amber-100 bg-amber-50/20 p-5 shadow-sm text-center">
              <p className="text-[10px] font-bold text-amber-600/95 uppercase tracking-wider">
            ...
            <div className="rounded-2xl border border-emerald-100 bg-emerald-50/20 p-5 shadow-sm text-center">
              <p className="text-[10px] font-bold text-emerald-600/90 uppercase tracking-wider">
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-premium flex flex-col justify-between">
          <div>
            ...
            <p className="text-[11px] text-slate-400 leading-normal mb-4">
              Visual representation of total interest overhead vs core loan
              capital.
            </p>
          </div>
          ...
          <div className="mt-4 rounded-xl bg-slate-50 p-4 border border-slate-100 flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">
                Interest Percentage Ratio
              </p>
              <p className="text-sm font-extrabold text-slate-700">
                {(
                  (calculations.totalInterest / calculations.totalPayment) *
                  100
                ).toFixed(1)}
                % of total payment
              </p>
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between bg-slate-50/50">
          <div>
            <h3 className="text-base font-bold text-slate-800 tracking-tight leading-none">
              Amortization Ledger Schedule
            </h3>
            <p className="text-xs text-slate-400 mt-1 leading-normal">
              Granular amortization table outlining breakdown of principal vs
              interest monthly deductions.
            </p>
          </div>
          <button
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 font-medium text-slate-600">
              {calculations.schedule.slice(0, 12).map((row) => (
                <tr
                  key={row.month}
                  className="hover:bg-slate-50/40 smooth-transition"
                >
              ...
              {calculations.schedule.length > 12 && (
                <tr className="bg-slate-50/20">
                  <td
                    colSpan={5}
                    className="px-6 py-4 text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-slate-50/40"
                  >
