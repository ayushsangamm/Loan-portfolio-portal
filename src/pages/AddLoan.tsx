...
import { calculateEMI, generateAmortizationSchedule } from '../utils/calculateEMI';
import { formatCurrency } from '../utils/formatCurrency';

// Zod Validation Schema
const addLoanSchema = z.object({
  borrowerName: z.string().min(3, 'Borrower name must be at least 3 characters long'),
  ...
  loanType: z.enum(['Personal', 'Business', 'Home', 'Education'] as const),
  purpose: z.string().min(5, 'Purpose description must be at least 5 characters long')
});

type LoanFormValues = z.infer<typeof addLoanSchema>;
...
export const AddLoan = () => {
  ...
  const {
    ...
    resolver: zodResolver(addLoanSchema) as any,
    defaultValues: {
      borrowerName: '',
      email: '',
      phone: '',
  ...
  const handleBorrowerSelect = (bId: string) => {
  ...
  const onSubmit = async (data: LoanFormValues) => {
    ...
    try {
      ...
      if (!bId) {
        ...
        const randomScore = creditScores[Math.floor(Math.random() * creditScores.length)];
        const cities = ['Mumbai', 'Delhi', 'Bengaluru', 'Pune', 'Hyderabad', 'Chennai'];
        const randomCity = cities[Math.floor(Math.random() * cities.length)];
        const employmentOptions = ['Employed', 'Self-Employed', 'Retired'] as const;
        const randomEmp = employmentOptions[Math.floor(Math.random() * employmentOptions.length)];
      ...
      const completeLoan = {
        ...
        tenure: data.tenure,
        status: 'Active' as const, // default active disbersed
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold tracking-tight text-slate-800 md:text-2xl leading-none">Loan Disbursement Wizard</h1>
        <p className="text-xs text-slate-400 mt-1.5 leading-normal">
          Validate and disburse new capital allocations onto the enterprise portfolio.
        </p>
      </div>

      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        {/* Form panel (Left 2 columns) */}
        <form onSubmit={handleSubmit(onSubmit)} className="lg:col-span-2 rounded-2xl border border-slate-200 bg-white p-6 shadow-premium space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Link Existing Profile (Optional)</label>
                <select
              ...
              {/* Name */}
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Borrower Name</label>
                <input
                  type="text"
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Phone Number (10 digits)</label>
                <input
                  type="text"
                  maxLength={10}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1">
                  ...
                  <span title="Values between ₹50k and ₹50L"><HelpCircle className="h-3.5 w-3.5 text-slate-350 cursor-pointer" /></span>
                </label>
                <input
                  type="number"
                  placeholder="100000"
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1">
                  ...
                  <span title="Values between 5% and 25%"><Percent className="h-3.5 w-3.5 text-slate-350 cursor-pointer" /></span>
                </label>
                <input
                  type="number"
                  step="0.1"
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1">
                  ...
                  <span title="Values between 6 and 240 months"><Clock className="h-3.5 w-3.5 text-slate-350 cursor-pointer" /></span>
                </label>
                <input
                  type="number"
                  placeholder="24"
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              {/* Type selection */}
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Loan Type Category</label>
                <select
                  {...register('loanType')}
                  className="mt-1.5 w-full rounded-xl border border-slate-250 bg-slate-50/50 px-3.5 py-2.5 text-xs font-semibold text-slate-700 outline-none hover:bg-slate-50 smooth-transition"
                >
              ...
              <div className="sm:col-span-2">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Statement of Purpose</label>
                <input
                  type="text"
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-900 text-white p-6 shadow-premium flex flex-col justify-between">
          <div className="space-y-6">
            <div className="flex items-center gap-3 border-b border-slate-800 pb-4">
              ...
              <div>
                ...
                <p className="text-[10px] font-semibold text-slate-400 tracking-wide uppercase">Realtime Calculations</p>
              </div>
            </div>
            ...
            <div className="space-y-4.5 text-xs">
              ...
              <div className="rounded-xl bg-slate-850 p-4 border border-slate-800">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Estimated Monthly EMI</span>
              ...
              <div className="flex justify-between border-b border-slate-850 pb-2.5">
                <span className="font-semibold text-slate-400">Principal Volume</span>
                <span className="font-bold text-slate-100">{formatCurrency(watchAmount)}</span>
              </div>

              {/* Interest volume */}
              <div className="flex justify-between border-b border-slate-850 pb-2.5">
                <span className="font-semibold text-slate-400">Total Interest Payable</span>
                <span className="font-bold text-emerald-400">+{formatCurrency(estimatedInterest)}</span>
              </div>

              {/* Aggregate value */}
              <div className="flex justify-between border-b border-slate-850 pb-2.5">
                <span className="font-semibold text-slate-400">Aggregate Repayments</span>
                <span className="font-bold text-slate-100">{formatCurrency(totalRepayment)}</span>
              </div>

              {/* Interest Rate summary */}
              <div className="flex justify-between">
                <span className="font-semibold text-slate-400">Annual Pricing (APR)</span>
                <span className="font-bold text-slate-100">{watchRate}% per annum</span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 rounded-xl bg-slate-850/60 p-4 border border-slate-850 text-[11px] text-slate-400 leading-normal flex items-start gap-2.5">
            ...
            <span>
              All estimates are calculated based on monthly compounding amortizations. Actual banking ledger terms might differ slightly depending on tax additions.
            </span>
          </div>
        </div>
      </div>
