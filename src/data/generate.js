...
for (let i = 1; i <= 50; i++) {
  ...
  // We can calculate this based on index to spread across years
  ...
  // Determine status
  // Spread status based on credit scores and dates
  let status = "Active";
  ...
  for (let m = 1; m <= tenure; m++) {
    ...
    // Populate payment history for completed months
  ...
  // If a loan is defaulted, let's make sure it has some late items too
  if (status === "Defaulted" && paymentHistory.length > 3) {
    // Mark one or two payments as late to make the history look authentic
  ...
  loans.push({
    ...
    repaymentSchedule: repaymentSchedule.slice(0, 24), // Keep it high performance by limiting display schedule
