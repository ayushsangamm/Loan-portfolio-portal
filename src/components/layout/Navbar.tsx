...
import { Menu, Bell, User, Clock, Calendar, AlertTriangle, AlertCircle, Sun, Moon } from 'lucide-react';
...
export const Navbar = ({ onOpenSidebar }: NavbarProps) => {
  ...
  const alerts = useMemo(() => {
    const list: { id: string; type: 'default' | 'pending'; title: string; desc: string }[] = [];
    loans.forEach((loan) => {
  ...
  const getPageTitle = () => {
    ...
    if (path.startsWith('/loans')) {
      if (path.includes('/new') || path === '/add-loan') return 'Disburse New Loan';
    ...
    <header className="sticky top-0 z-40 flex h-16 w-full items-center justify-between border-b border-slate-200 bg-white/80 px-6 backdrop-blur-md">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <div className="hidden items-center gap-1.5 text-[11px] font-medium text-slate-400 md:flex">
            ...
            <span>/</span>
            <span className="text-slate-500 font-semibold">{getPageTitle()}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4.5">
        ...
        <div className="hidden items-center gap-3 rounded-xl border border-slate-200/60 bg-slate-50/50 px-3.5 py-1.5 text-xs text-slate-600 shadow-2xs md:flex">
          ...
          <span className="font-medium text-slate-500">{format(time, 'EEE, dd MMM yyyy')}</span>
          <span className="h-3.5 w-[1px] bg-slate-200" />
          <Clock className="h-3.5 w-3.5 text-brand-500" />
          <span className="font-bold text-slate-700 tracking-wide">{format(time, 'hh:mm:ss a')}</span>
        </div>
        ...
        <div className="relative">
          <button 
            ...
            className={`relative flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 shadow-2xs hover:bg-slate-50 hover:text-slate-800 smooth-transition cursor-pointer ${
              showNotifications ? 'bg-slate-100 border-slate-350 text-slate-850' : ''
            }`}
          >
            ...
            <>
              ...
              <div className="absolute right-0 mt-2.5 z-50 w-80 rounded-2xl border border-slate-200 bg-white p-2.5 shadow-premium animate-fade-in">
                <div className="px-3 py-2 border-b border-slate-100 flex items-center justify-between">
                  <span className="text-xs font-black text-slate-800">System Priority Alerts</span>
                ...
                <div className="max-h-64 overflow-y-auto mt-2 space-y-1.5">
                  {alerts.length === 0 ? (
                    <div className="py-8 text-center text-slate-400">
                      ...
                      <p className="text-[9px] mt-0.5">No critical issues to review</p>
                    </div>
                  ) : (
                    alerts.map((alert) => {
                      const Icon = alert.type === 'default' ? AlertTriangle : AlertCircle;
                      return (
                        <div 
                          key={alert.id}
                          onClick={() => {
                          ...
                          className={`flex items-start gap-3 p-2.5 rounded-xl border border-transparent hover:border-slate-200/60 hover:bg-slate-50 cursor-pointer smooth-transition ${
                            alert.type === 'default' ? 'bg-rose-50/20' : 'bg-amber-50/20'
                          }`}
                        >
                          <div className={`mt-0.5 h-7 w-7 rounded-lg flex items-center justify-center ${
                            alert.type === 'default' ? 'bg-rose-100 text-rose-600' : 'bg-amber-100 text-amber-600'
                          }`}>
                            <Icon className="h-4 w-4" />
                          </div>
                          <div className="flex-1 min-w-0">
                            <h5 className="text-[11px] font-bold text-slate-800 leading-tight">{alert.title}</h5>
                            <p className="text-[10px] text-slate-500 leading-normal mt-0.5 truncate">{alert.desc}</p>
                          </div>
                        </div>
                      );
                    })
        ...
        <div className="flex items-center gap-3 pl-1.5 border-l border-slate-200/80">
          ...
          <div className="hidden text-left md:block">
            <h4 className="text-xs font-bold text-slate-700 leading-tight">Ayush Admin</h4>
            <p className="text-[10px] font-semibold text-brand-600 tracking-wide uppercase">Portfolio Officer</p>
          </div>
        </div>
      </div>
    </header>
