...
export const LoanTrendChart = ({ loans }) => {
  const chartData = useMemo(() => {
    ...
    loans.forEach((loan) => {
      ...
      const q = quarters.find(
        (quarter) =>
          loanDate >= new Date(quarter.start) &&
          loanDate <= new Date(quarter.end),
      );
    ...
    <div className="h-80 w-full rounded-2xl border border-slate-200/80 bg-white p-6 shadow-premium">
      <div className="mb-4">
        <h3 className="text-sm font-bold tracking-tight text-slate-800 md:text-base">
          Capital Disbursement Trends
        </h3>
        <p className="text-xs text-slate-400 leading-normal">
          Quarterly overview of portfolio loan value disbursed (2023 – 2025)
        </p>
      </div>
