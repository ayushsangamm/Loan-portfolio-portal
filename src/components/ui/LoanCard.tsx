...
export const LoanCard = ({ title, value, icon: Icon, description, color }: LoanCardProps) => {
  const themes = {
    blue: {
  ...
  return (
    ...
    `}>
      ...
      <div className={`absolute -right-16 -top-16 h-36 w-36 rounded-full bg-gradient-to-br opacity-50 blur-xl ${currentTheme.bg}`} />

      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold tracking-tight text-slate-500">{title}</h3>
        <div className={`flex h-10 w-10 items-center justify-center rounded-xl shadow-2xs ${currentTheme.iconBg}`}>
      ...
      <div className="mt-4">
        ...
        <div className="mt-2.5 flex items-center">
          <span className={`inline-flex rounded-md border px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${currentTheme.badge}`}>
            {description}
          </span>
        </div>
      </div>
